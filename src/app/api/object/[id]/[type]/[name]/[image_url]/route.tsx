...
function jsonToStrings(json: { [x: string]: any; }, prefix = ''): string[] {
    let strings: string[] = [];
    for (const key in json) {
        if (typeof json[key] === 'object' && json[key] !== null) {
            strings = strings.concat(jsonToStrings(json[key], `${prefix}${key}.`));
        } else {
            strings.push(shortenType(`${key}: ${json[key]}`));
        }
    }
    return strings;
}
...
export async function GET(req: NextRequest, context: { params: Params }) {
    ...
    const type = shortenType(context.params.type)
    const name = minType(context.params.name)
    const image_url = context.params.image_url
    // const fields = context.params.fields
    // const parsedFields = JSON.parse(fields);
    // const fieldStrings = jsonToStrings(parsedFields);
 ...
 L84.73,43.84z" />
            ...
            <pattern id="bg" patternUnits="userSpaceOnUse" x="0" y="0" width="120" height="120">
                <image href="${image_url == "Unknown" ? "" : imageBase64}" width="120" height="120" />
            </pattern>
            <mask id="text">
                <text fill="#FFFFFF" font-size="12" text-anchor="middle">
                    <tspan x="50%" y="15%" font-size="7">${name}</tspan>
                    <tspan x="51%" y="50%" font-size="18">${shorten_id}</tspan>
                    <tspan x="50%" y="95%" font-size="4">${type}</tspan>
